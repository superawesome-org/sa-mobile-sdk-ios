...
class AwesomeAdsApiTests: XCTestCase {
    ...
    func test_moyaProvider_canGetAndParse_adResponse_1() throws {
        ...
        expect(ad.isFill).to(equal(false))
        expect(ad.isFallback).to(equal(false))
        expect(ad.campaignType).to(equal(0))
        expect(ad.isHouse).to(equal(false))
        expect(ad.safeAdApproved).to(equal(true))
        expect(ad.showPadlock).to(equal(true))
        expect(ad.lineItemId).to(equal(931))
        expect(ad.test).to(equal(false))
        expect(ad.app).to(equal(1484))
        expect(ad.device).to(equal("web"))
        expect(ad.creative.clickUrl).to(equal("https://superawesome.tv"))
        ...
        expect(ad.creative.details.placementFormat).to(equal("video"))
    ...
    func test_moyaProvider_canGetAndParse_adResponse_2() throws {
        ...
        expect(ad.safeAdApproved).to(equal(true))
        expect(ad.showPadlock).to(equal(true))
        expect(ad.lineItemId).to(equal(138))
        expect(ad.test).to(equal(false))
        expect(ad.app).to(equal(105))
        expect(ad.device).to(equal("web"))
        expect(ad.creative.clickUrl).to(equal("http://superawesome.tv"))
        expect(ad.creative.details.duration).to(equal(0))
        expect(ad.creative.details.height).to(equal(50))
        expect(ad.creative.details.placementFormat).to(equal("mobile_display"))
        expect(ad.creative.details.tag).to(beNil())
        ...
        print(ad.creative)
        expect(ad.creative.format).to(equal(.imageWithLink))
