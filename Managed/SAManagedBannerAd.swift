...
@objc(SAManagedBannerAd)
public class ManagedBannerView: UIView, Injectable {
    ...
    @objc
    private func play(_ placementId: Int) {
        logger.info("play()")
        addWebView()
        let _ =  webView?.loadAdViaJsScrpit(placementId: placementId, adRequest: makeAdRequest())
...
extension ManagedBannerView: WebViewDelegate {
    func webViewOnStart() {
