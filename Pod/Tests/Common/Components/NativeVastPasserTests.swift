...
class NativeVastPasserTests: XCTestCase {
    ...
    func test_parse_response_sample() throws {
        // Given
        let parser = NativeVastPasser(connectionProvider: ConnectionProviderMock())

        // When
        let vast = parser.parse(xmlFile("sample"))

        // Then
        expect(vast?.url).to(equal("https://video-transcoded-api-main-awesomeads.sacdn.net/MB7k7pTym4lGaqt3wd167TLCOMg6Lh4P.m3u8"))
        ...
        expect(vast?.media.count).to(equal(4))
    ...
    func test_parse_response1() throws {
        ...
        expect(vast?.media.count).to(equal(11))
