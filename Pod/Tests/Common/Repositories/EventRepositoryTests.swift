...
class EventRepositoryTests: XCTestCase {
    ...
    func test_givenSuccess_impressionCalled_returnsSuccess() throws {
        ...
        repository.impression(request: makeEventRequest(.impressionDownloaded)) { result in
            self.result = result
            expectation.fulfill()
        }
    ...
    func test_givenSuccess_clickCalled_returnsSuccess() throws {
        ...
        // When
        let expectation = self.expectation(description: "request")
        repository.click(request: makeEventRequest()) { result in
            self.result = result
            expectation.fulfill()
        }
        waitForExpectations(timeout: 2.0, handler: nil)
    ...
    func test_givenSuccess_videoClickCalled_returnsSuccess() throws {
        ...
        // When
        let expectation = self.expectation(description: "request")
        repository.videoClick(request: makeEventRequest()) { result in
            self.result = result
            expectation.fulfill()
        }
        waitForExpectations(timeout: 2.0, handler: nil)
    ...
    func test_givenSuccess_eventCalled_returnsSuccess() throws {
        ...
        // When
        let expectation = self.expectation(description: "request")
        repository.event(request: makeEventRequest()) { result in
            self.result = result
            expectation.fulfill()
        }
        waitForExpectations(timeout: 2.0, handler: nil)
